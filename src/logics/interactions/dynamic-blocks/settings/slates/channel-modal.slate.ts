...
export const channelesModal = async (options: {
  id: string
  i: number
  ...
  const { id, i, selectedSpaceText, selectedTeamText, selectedObjectId, selectedChannelText } = options
  return {
    rootBlock: `${id}channel`,
    blocks: [
      {
        id: `${id}.${i}.card`,
        name: 'Card',
